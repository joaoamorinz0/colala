...
export default function EstadiaAdminPage() {
  ...
  useEffect(() => {
    const loadPlaces = async () => {
      try {
        ...
        const data = await getPlacesByCategorySlug(client, ESTADIA_SLUG);
        setPlaces(data);
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Erro ao carregar estadias",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Tem certeza que deseja deletar esta estadia?")) return;
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Erro ao deletar estadia");
    }
  };
  ...
  return (
    <AdminLayout>
      <PageHeader
        title="Estadias"
        description="Gerencie todas as estadias cadastradas"
        action={
          <Link
            href={"/admin/estadias/new" as never}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-3">
                          <Link
                            href={`/admin/estadias/${place.id}/edit` as never}
