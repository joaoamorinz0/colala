...
export default function NewEstadiaPage() {
  ...
  const handleSubmit = async (data: Omit<Place, "id" | "created_at">) => {
    ...
    try {
      ...
      router.push("/admin/estadias" as never);
    } catch (error) {
      throw error;
    } finally {
      setIsLoading(false);
  ...
  return (
    <AdminLayout>
      ...
      <PageHeader
        title="Nova Estadia"
        description="Adicione uma nova estadia ao catálogo"
      />

      <PlaceForm
        onSubmit={handleSubmit}
        isLoading={isLoading}
        defaultCategorySlug="estadia"
      />
    </AdminLayout>
  );
}
